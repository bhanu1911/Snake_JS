...
{
    //Canvas on which the game will be working.
    var canvas = document.createElement('canvas'),
  
    //ctx will be used as the context variable from here on
    ctx = canvas.getContext('2d'),
    //Variable declarations and initializations for the most important variables we have to consider i.e. score and level
    score = 0,
    level = 0,
    //direction is the direction in which we want the snake to move 
    direction = 0,
    snake = new Array(3),
    //This is the "state" of the game, i.e., this dictates whether the game works or not based on some specific conditions
    active = true,
    //We don't need 60fps for this stuff.
    speed = 500;
    ...
    //We can adjust this ourselves as well (dimensions of the canvas)
    canvas.width = 204;
    canvas.height = 224;

    //Here we start with the body of the document, which is an element of an array 
    var body = document.getElementsByTagName('body')[0];
    //Append the canvas as a child to body
    ...
    //drawGame is required to draw the entirety of the game 
    //Contains methods pertaining to the game, the programming "logic" of the game, as it were
    drawGame();

    //The logic here is, we're navigating the snake in the canvas with keys and we do not want two counter-logical keys to be pressed within a short duration of the other (i.e. front and back just after the other). Hence, we add conditional statements to the listener which carries as arguments the 'keydown' object and the function taking in 'e' as an argument, which takes in the  
...
//SNAKE IS AN ARRAY OF OBJECTS CONTAINING THE X AND Y COORDINATES!!! !!! 
    function drawGame() 
    {
        // Clear the canvas
        ...
        for (var i = snake.length - 1; i >= 0; i--) {
            ...
            if (i === 0) {
                ...
                if (map[snake[0].x][snake[0].y] === 1) {
                    ...
                    //!!!!OBJECT.push(<values>) is a very important function, for it adds the values to the base object
